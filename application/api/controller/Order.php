            ...
            if($goods) {
                ...
                foreach ($goods as $key=>&$val) {
                    ...
                    if ($val['has_refund']){
                        ...
                        $order['orderBeans'][$key]['refund_id'] = Db::name('order_refund')->where($order_refund_where)->value('refund_id');
    ...
    public function apply_refund(){
        if (Request::instance()->isPost()) {
            ...
            if ($result) {
                ...
                Db::name('order_merchants')->where(['order_merchants_id'=>$order['order_merchants_id']])->update(['refund_state'=>1]);
                success("申请信息提交成功");
            } else {
                error("申请信息提交失败");
            }
