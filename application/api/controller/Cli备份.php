...
//                                'authName'=>""
    ...
    public function order_goods_settlement(){
        ...
        $orderWhere['a.receive_time'] = ['between',['2017-12-01 00:00:00',date('Y-m-d H:i:s',$time)]];
        //$orderWhere['a.receive_time'] = ['between',['2017-12-01 00:00:00',date("Y-m-d H:i:s",$time-10*24*3600)]];
            ...
            foreach ($order as $k => $v) {
                ...
                foreach ($orderGoods as $key => $val) {
                    ...
                    if ($settlement_price > 0) {
                        ...
                        if($sell_scale>=$system['spread_scale1'] && !empty($v['spread_tv'])){
                        ...
                        if ($val['seller']) { //有销售者
                            ...
                            if ($anchor) {
                                if ($anchor['tv_id']) {//电视台主播
                                    ...
                                    $goods_settlement[$i]['seller_amount'] = $settlement_price * $v['tv_sell_scale']/100;
                                    if ($v['tv_sell_scale'] > 0) {
                                        ...
                                        if ($tv_relation) {
                                            if ($tv_relation['shop_ratio'] > 0) {//自己电视台结算
                                                $tv_money1 = $settlement_price * $tv_relation['shop_ratio'] * $v['tv_sell_scale'] / 10000;
                                            ...
                                            if ($tv_relation['city_shop_ratio'] > 0 && $tv_relation['city_tv_id']) { //城市市电视台结算
                                                $tv_money2 = $settlement_price * $tv_relation['city_shop_ratio'] * $v['tv_sell_scale'] / 10000;
                                            ...
                                            if ($tv_relation['province_shop_ratio'] > 0 && $tv_relation['province_tv_id']) { //省级电视台结算
                                                $tv_money3 = $settlement_price * $tv_relation['province_shop_ratio'] * $v['tv_sell_scale'] / 10000;
                                        ...
                                        $goods_settlement[$i]['other_amount'] = $settlement_price * $v['tv_sell_scale']/100 - $tv_money1 - $tv_money2 - $tv_money3;
                                    }

                                    $merchants_amount = $settlement_price - $goods_settlement[$i]['spread_amount'] - $settlement_price * $v['tv_sell_scale']/100 - $goods_settlement[$i]['platform_amount']-$goods_settlement[$i]['spread_tv_amount'];
                                ...
                                if ($val['sale_ratio'] > 0) {//其他商家推广
                                    $goods_settlement[$i]['seller_ratio'] = $sale_ratio;//推广者提成比例
                                    $seller_amount = sprintf('%.2f', $settlement_price * $system['tv_sell_scale'] / 100);
