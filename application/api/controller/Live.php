    ...
    public function into_live(){
        ...
        if ($live){
            ...
            if (in_array($user["member_id"],$ids)){
                ...
                if ($live_number){
                    DB::name('Live')->where(['live_id' =>$live_id])->setDec('watch_nums');
                    DB::name('Live_number')->where(['live_number_id'=>$live_number['live_number_id']])->delete();
                }
            }
        }
    ...
    */
    ...
    public function out_live(){
       $user = $this->checklogin();
        ...
        $user_id2 = DB::name('Live')->where(["live_id"=>$live_id])->value('user_id');
        if (DB::name('Live_number')->comment('删除记录')->where(['live_id'=>$live_id,'user_id'=>$user_id2,'user_id2'=>$user["member_id"]])->delete()){
            DB::name('Live')->where(['live_id' =>$live_id])->setDec('watch_nums');
            success('成功退出!');
        }else{
            error('退出失败!');
