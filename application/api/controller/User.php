...
class User extends Common{
    ...
    public function user_info(){
        if (Request::instance()->isPost()) {
            ...
            //送出
            $give_count = DB::name('member')->where(['member_id' => $user['member_id']])->value('give_gift_count');
...
//                $map['intime'] = ['gt',$intime];
                ...
                $page = ceil($count/$pagesize);
                $list = Db::name('system_notice')->field('intime,content as message,title')
     ...
     */
     ...
     public function edit_phone(){
         ...
         $user = DB::name("member")->where(["phone"=>$param['mobile']])->find();
         if($user)          error("手机号已存在");
         $default_verify = DB::name("system")->where(["id"=>1])->value("default_verify");
         //判断验证码是否有效期
         if($param['yzm'] != $default_verify) {
             $result = DB::name("Mobile_sms")->where(["mobile" => $param["mobile"], "code" => $param["yzm"]])->order("intime desc")->find();
             if (!$result) {
                 error("验证码不正确");
             }
             $state = $result["state"];
         ...
         $result = Db::name('member')->where(['member_id'=>$member['member_id']])->update(['phone'=>$param['mobile']]);
         if($result){
             success('手机号绑定成功');
         }else{
             error('手机号绑定失败');
     ...
     {
         $type = input('type','1');
         $db = DB::name('notice')->where(['state'=>1,'type'=>$type])->order('is_top asc')->select();
         success($db);
     }
