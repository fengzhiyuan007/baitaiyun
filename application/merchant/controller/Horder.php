...
class Horder extends Base
{
    ...
    protected function get_order($map,$num){
        ...
        $list = Db::name('order_merchants')->alias('a')
            ...
            ->join('th_member b','a.member_id = b.member_id','left')
...
//                }
    ...
    public function change_refund_state(){
        if(Request::instance()->isAjax()){
            ...
            $state = input('state');
            $check = Db::name('order_refund')->where(['refund_id'=>$refund_id])->find();
            if($check) {
                ...
                $work .='原订单状态是:'.$tag;
                if($state == 'end'){
                    $order_merchants = Db::name('order_merchants')->where(['order_merchants_id'=>$check['order_merchants_id']])->find();
                ...
                $result = Db::name('order_refund')->where(['refund_id' => $check['refund_id']])->update($data);
                if ($result) {
