...
class Caiwu extends Base{
    ...
    public function to_withdraw_ticket(){
        ...
        $merchant['money'] = $merchant['e_ticket'] * $cash_money;
        if(request()->isAjax()) {
            if($merchant['e_ticket']<100)       error('钻石数量少于100');
            $data['e_ticket'] = 0;
            $data['amount'] = $merchant['amount'] + $merchant['money'];
            $result = Db::name('member')->where(['member_id'=>$merchant['member_id']])->update($data);
            if($result){
    ...
    public function to_withdraw_money(){
        ...
        $cash_money =  $system['convert_scale4']/$system['convert_scale3'];
        $merchant['money'] = $merchant['e_ticket'] * $cash_money;
        $bank = Db::name('bank_card')->where(['member_id'=>$merchant['member_id']])->find();
        if(request()->isAjax()) {
            if(!$bank)                                      error('请先填写提现账户信息');
            $money = input('money');
            if($money>$merchant['amount'] || $money<0)      error('提现余额错误');
            $data['user_type'] = '1';
            ...
            $result = Db::name('member')->where(['member_id'=>$merchant['member_id']])->setDec('amount',$money);
            if(!$result){
                Db::rollback();
                error("余额提现申请失败");
            }else{
                Db::commit();
                success(['info'=>'余额提现申请成功','url'=>url('Caiwu/to_withdraw_money')]);
            }
