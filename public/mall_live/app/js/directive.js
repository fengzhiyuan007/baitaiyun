...
.directive("domToggle", [function() {
  return {
    ...
    link: function($scope, element, attributes) {
      element.click(function() {
        console.log($scope.domType+'/'+$scope.domName)
        if($scope.domType == 'close'){
          angular.element($scope.domName).hide();
        }else{
          angular.element($scope.domName).show();
        }
      });
    }
  }
...
.directive('orderOperation',function($http,$cookieStore,myFactory){
  return {
  	...
  	link: function($scope,$element, $attributes){
        ...
        }else if($scope.orderType==5){ // 催单
          $scope.api = 'api/Order/hurry_order';
        }
  			$http.post(
		      url + $scope.api,$.param({
    ...
    link: function($scope,$element, $attributes){
      $element.click(function(){
        ...
        $http.post(
          ...
          myFactory.promptFun(err.data.error.message,1300);
        });
      })
    }
  }
